...
function FormAddress({ onUpdate = () => {} }) {
  ...
  useEffect(() => {
    async function fetchAddress() {
      ...
      dispatch({ type: "UPDATE_FULL_ADDRESS", payload: result });
  ...
  return (
    <Grid container spacing={2} alignItems="center">
      ...
      {[
        { label: "Rua", xs: 9, name: "address", inputRef: addressField },
        { label: "Número", xs: 3, name: "number", inputRef: numberField },
        { label: "Complemento", xs: 12, name: "complement" },
        { label: "Cidade", xs: 9, name: "city" },
        { label: "Estado", xs: 3, name: "state" },
      ].map((field) => (
        <TextField
          {...field}
          key={field.name}
